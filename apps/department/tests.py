...
class DepartmentAPITests(TestCase):
    def setUp(self):
        ...
        self.project = Project.objects.create(
            name="Project A", owner=self.org_owner, department=self.department
        )
    ...
    def test_create_department_by_admin_success(self):
        ...
        url = reverse("dept-list", kwargs={"oid": self.org.id})
        data = {"name": "Sales", "description": "Sales Division"}
        response = client.post(url, data, format="json")
        self.assertEqual(response.status_code, 201)
    ...
    def test_retrieve_department_success(self):
        client = self.get_auth_client(self.org_member)
        url = reverse(
            "dept-detail", kwargs={"oid": self.org.id, "pk": self.department.id}
        )
        response = client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_department_projects_success(self):
        client = self.get_auth_client(self.org_member)
        url = reverse(
            "dept-project-list", kwargs={"oid": self.org.id, "pk": self.department.id}
        )
        response = client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data), 1)
