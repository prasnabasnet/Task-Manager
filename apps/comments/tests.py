...
class CommentAPITests(TestCase):
    def setUp(self):
        ...
        # URL
        self.url = reverse('task-comment-list-create', kwargs={'pid': self.project.id, 'tid': self.task.id})
    ...
    def test_post_comment_success(self):
        ...
        data = {'body': 'This is a test comment.', 'parent': None}
    ...
    def test_post_comment_with_mention(self):
        client = self.get_auth_client(self.dev_user)
        # Mention pm_user
        data = {'body': 'Hey @pm_user please look at this', 'parent': None}
        response = client.post(self.url, data, format='json')
        self.assertEqual(response.status_code, 201)
    ...
    def test_threaded_replies(self):
        ...
        parent = Comment.objects.create(task=self.task, author=self.pm_user, body='Parent comment')
    ...
    def test_edit_own_comment(self):
        comment = Comment.objects.create(task=self.task, author=self.dev_user, body='Old comment')
        client = self.get_auth_client(self.dev_user)
    ...
    def test_delete_own_comment(self):
        comment = Comment.objects.create(task=self.task, author=self.dev_user, body='Comment to delete')
        client = self.get_auth_client(self.dev_user)
        detail_url = reverse('comment-detail', kwargs={'pk': comment.id})
    ...
    def test_non_project_member_denied(self):
        ...
        response_post = client.post(self.url, {'body': 'Denied comment'}, format='json')
        self.assertEqual(response_post.status_code, 403)
        response_get = client.get(self.url)
        self.assertEqual(response_get.status_code, 403)

        # Denied from retrieving comment detail
        comment = Comment.objects.create(task=self.task, author=self.dev_user, body='Secret comment')
        detail_url = reverse('comment-detail', kwargs={'pk': comment.id})
